...
| Runtime MCP servers (in container)        | ✅ shipped | `mcp-chain` (3100), `mcp-compiler` (3101), `mcp-deployer` (3102), `mcp-wallet` (3103), `mcp-memory` (3104) baked into the runner image and supervised by `entrypoint.sh`.                                                                                                                                                         |
| Control-plane `tool_exec` proxy           | ✅ shipped | HTTP proxy routes `chain` / `compiler` / `deployer` / `wallet` / `memory` to in-container ports. `terminal` / `mesh` still return "not implemented".                                                                                                                                                                              |
...
| Frontend shell                            | ✅ shipped | SvelteKit 2 + Svelte 5; chat rail / editor / preview / terminal panes; SSE agent stream; workspace boot polling; SIWE login; preview pane iframes `previewUrl` when set.                                                                                                                                                          |
...
| `deploy_og_chain` (0G Chain)              | ✅ shipped | viem + OG_TESTNET_CHAIN (chainId 16602) in `mcp-deployer`. Tool name fixed (`deploy_0g_chain` → `deploy_og_chain`). System prompt updated to prevent agent from calling `chain.start_node` for 0G deploys.                                                                                                                        |
| `mcp-mesh` (AXL)                          | 🔴 missing | Type contracts frozen in `packages/types/src/mcp/mesh.ts`. Package dir exists but empty. AXL binary not running. Stretch goal for Day 8.                                                                                                                                                                                          |
...
**Current status (May 1):** POV-1 is complete. 0G inference, 0G Storage KV backend, and 0G Chain deployment are all shipped. Critical path is now the demo scaffold: a richer contract with a demonstrable self-healing arc that hits all three 0G touchpoints. See `todo.md` → "Demo Scaffold" section. ⚠️ Note: 0G Compute testnet serves qwen2.5 7b — the self-healing repair loop was designed with qwen3.6 35b a3b. Validate the arc with the weaker model before recording.
