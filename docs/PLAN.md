...
| Control-plane `tool_exec` proxy           | ✅ shipped | HTTP proxy routes `chain` / `compiler` / `deployer` / `wallet` / `memory` / `terminal` to in-container ports. Only `mesh` still returns "not implemented".                                                                                                                                                                                                    |
...
| Chat history persistence                  | ✅ shipped | All `AgentEvent`s persisted as JSONL to `.crucible/chat.jsonl` via `recordChatEvent` hook in `publishAgentEvent`, with token deltas coalesced to prevent file bloat. `GET /api/workspace/:id/chat/history` returns the full history; frontend hydrates `AgentStream.events` on workspace load so users resume conversations without losing context on reload. |
