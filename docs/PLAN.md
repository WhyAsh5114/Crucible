...
**Current status (May 2):** POV-1 and POV-2 are complete. The self-healing repair loop is shipped and validated end-to-end on glm-5, minimax-b2.5, and deepseek-v4-pro (full deposit → withdraw → revert → snapshot → trace → recall → patch → verify → mine → withdraw arc). Critical path is now the demo video and submission form.
